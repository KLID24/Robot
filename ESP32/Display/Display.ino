...
#include <FirebaseESP32.h>   // новая библиотека
...
// Объявляем объекты для Firebase
FirebaseData fbdo;
FirebaseAuth auth;
...
void setup() {
  ...
  auth.user.email = "";       // если нет логина/пароля
  auth.user.password = "";
  ...
  // Новый способ чтения данных из RTDB
  if (firebase.getString(fbdo, String(USER_PATH) + "/name")) {
...
void loop() {
  // Можно добавить периодическое обновление данных
}
